...
class TechnicalAnalyzer:
    ...
    def analyze_comprehensive(self, data):
        ...
        try:
            ...
            analysis['bb_position'] = (close - analysis['lower_band']) / (analysis['upper_band'] - analysis['lower_band'])
    ...
    def calculate_technical_score(self, analysis):
        ...
        try:
            ...
            ma10_latest = analysis.get('ma10', pd.Series()).iloc[-1] if len(analysis.get('ma10', pd.Series())) > 0 else 0
            ma20_latest = analysis.get('ma20', pd.Series()).iloc[-1] if len(analysis.get('ma20', pd.Series())) > 0 else 0
            ...
            # MACD评分
            macd_latest = analysis.get('macd', pd.Series()).iloc[-1] if len(analysis.get('macd', pd.Series())) > 0 else 0
            signal_latest = analysis.get('signal', pd.Series()).iloc[-1] if len(analysis.get('signal', pd.Series())) > 0 else 0
            ...
            # 布林带评分
            bb_position = analysis.get('bb_position', pd.Series()).iloc[-1] if len(analysis.get('bb_position', pd.Series())) > 0 else 0.5
...
class SentimentAnalyzer:
    ...
    def analyze_fund_flow_sentiment(self, stock_code):
        ...
        try:
            ...
            if not stock_flow.empty:
                # 解析主力净流入
                main_flow_str = stock_flow.iloc[0]['今日主力净流入-净额']
    ...
    def analyze_sector_sentiment(self):
        ...
        try:
            ...
            # 计算平均流入
            positive_count = len(sector_flow[sector_flow['今日主力净流入-净额'].str.contains(r'^[^-]', na=False)])
    ...
    def analyze_market_emotion(self):
        ...
        try:
            ...
            if len(sh_index) >= 5:
                recent_changes = sh_index['涨跌幅'].tail(5).mean()
...
class RiskManager:
    ...
    def calculate_beta(self, stock_returns):
        ...
        try:
            ...
            market_returns = market_data['涨跌幅'].pct_change().dropna()
...
class AIPredictor:
    ...
    def predict_trend(self, data, technical_data):
        ...
        try:
            ...
            # 获取最新的技术指标
            latest_rsi = technical_data.get('rsi', pd.Series()).iloc[-1] if len(technical_data.get('rsi', pd.Series())) > 0 else 50
            latest_macd = technical_data.get('macd', pd.Series()).iloc[-1] if len(technical_data.get('macd', pd.Series())) > 0 else 0
            latest_signal = technical_data.get('signal', pd.Series()).iloc[-1] if len(technical_data.get('signal', pd.Series())) > 0 else 0
            latest_bb_pos = technical_data.get('bb_position', pd.Series()).iloc[-1] if len(technical_data.get('bb_position', pd.Series())) > 0 else 0.5
            ...
            # 中期预测 (1-2周)
            medium_term_score = 0
            ma5 = technical_data.get('ma5', pd.Series()).iloc[-1] if len(technical_data.get('ma5', pd.Series())) > 0 else 0
            ma20 = technical_data.get('ma20', pd.Series()).iloc[-1] if len(technical_data.get('ma20', pd.Series())) > 0 else 0
            ma60 = technical_data.get('ma60', pd.Series()).iloc[-1] if len(technical_data.get('ma60', pd.Series())) > 0 else 0
            ...
            # ADX趋势强度
            adx = technical_data.get('adx', pd.Series()).iloc[-1] if len(technical_data.get('adx', pd.Series())) > 0 else 20
            if adx > 40:  # 强趋势
            ...
            price_trend = (close_prices.iloc[-1] - close_prices.iloc[-20]) / close_prices.iloc[-20] if len(close_prices) >= 20 else 0
...
class EnhancedStockAnalyzer(QMainWindow):
    ...
    def update_technical_display(self):
        ...
        try:
            ...
            # 更新指标数值
            ma5 = technical_data.get('ma5', pd.Series()).iloc[-1] if len(technical_data.get('ma5', pd.Series())) > 0 else 0
            ma10 = technical_data.get('ma10', pd.Series()).iloc[-1] if len(technical_data.get('ma10', pd.Series())) > 0 else 0
            ma20 = technical_data.get('ma20', pd.Series()).iloc[-1] if len(technical_data.get('ma20', pd.Series())) > 0 else 0
            rsi = technical_data.get('rsi', pd.Series()).iloc[-1] if len(technical_data.get('rsi', pd.Series())) > 0 else 0
            macd = technical_data.get('macd', pd.Series()).iloc[-1] if len(technical_data.get('macd', pd.Series())) > 0 else 0
            k_value = technical_data.get('k', pd.Series()).iloc[-1] if len(technical_data.get('k', pd.Series())) > 0 else 0
    ...
    def generate_technical_signals(self, technical_data):
        ...
        try:
            ...
            # RSI信号
            rsi = technical_data.get('rsi', pd.Series()).iloc[-1] if len(technical_data.get('rsi', pd.Series())) > 0 else 50
            if rsi > 80:
            ...
            # 布林带信号
            bb_position = technical_data.get('bb_position', pd.Series()).iloc[-1] if len(technical_data.get('bb_position', pd.Series())) > 0 else 0.5
            if bb_position > 0.8:
    ...
    def plot_technical_chart(self):
        ...
        try:
            ...
            if 'upper_band' in technical_data and 'lower_band' in technical_data:
                ...
                ax1.fill_between(dates, technical_data['upper_band'], technical_data['lower_band'], alpha=0.1, color='gray')
    ...
    def update_ai_display(self):
        ...
        try:
            ...
            self.short_term_pred.setText(f"{short_term.get('direction', '--')} (置信度: {short_term.get('confidence', 0):.1%})")
            self.medium_term_pred.setText(f"{medium_term.get('direction', '--')} (置信度: {medium_term.get('confidence', 0):.1%})")
            self.long_term_pred.setText(f"{long_term.get('direction', '--')} (置信度: {long_term.get('confidence', 0):.1%})")
            
            # 计算综合置信度
            avg_confidence = (short_term.get('confidence', 0) + medium_term.get('confidence', 0) + long_term.get('confidence', 0)) / 3
    ...
    def generate_ai_advice(self, prediction_data):
        ...
        try:
            ...
            confidences = [short_term.get('confidence', 0), medium_term.get('confidence', 0), long_term.get('confidence', 0)]
    ...
    def plot_ai_chart(self):
        ...
        try:
            ...
            # 根据预测生成价格趋势
            short_term = prediction_data.get('short_term', {})
            trend_factor = 0.001 if short_term.get('direction') == '上涨' else -0.001 if short_term.get('direction') == '下跌' else 0
            
            future_prices = []
            current_price = last_price
            for i in range(30):
                # 添加随机波动
                noise = np.random.normal(0, last_price * 0.02)
                current_price = current_price * (1 + trend_factor + noise / last_price)
                future_prices.append(current_price)
            
            # 绘制预测价格
            ax.plot(future_dates, future_prices, '--', label='AI预测趋势', linewidth=2, color='red', alpha=0.7)
            
            # 添加置信区间
            confidence = short_term.get('confidence', 0.5)
            upper_bound = np.array(future_prices) * (1 + 0.1 * (1 - confidence))
            lower_bound = np.array(future_prices) * (1 - 0.1 * (1 - confidence))
            
            ax.fill_between(future_dates, upper_bound, lower_bound, alpha=0.2, color='red', label='置信区间')
            
            ax.set_title('AI价格预测图')
            ax.legend()
            ax.grid(True, alpha=0.3)
    ...
    def export_analysis_report(self):
        ...
        if not self.analysis_data or not self.current_stock_code:
            QMessageBox.warning(self, "警告", "请先完成股票分析")
            return
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                self, "保存分析报告", 
                f"股票分析报告_{self.current_stock_code}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                "文本文件 (*.txt)"
            )
            ...
            QMessageBox.information(self, "成功", f"分析报告已保存至: {file_path}")
    ...
    def generate_analysis_report(self):
        ...
        try:
            report = []
            report.append("=" * 60)
            report.append(f"股票量化分析报告")
            report.append("=" * 60)
            report.append(f"股票代码: {self.current_stock_code}")
            ...
            if technical_data:
                report.append("【技术分析】")
                tech_score = technical_data.get('technical_score', 0)
                report.append(f"技术评分: {tech_score:.0f}/100")
                
                if 'ma5' in technical_data and len(technical_data['ma5']) > 0:
                    report.append(f"MA5: {technical_data['ma5'].iloc[-1]:.2f}")
                if 'ma20' in technical_data and len(technical_data['ma20']) > 0:
                    report.append(f"MA20: {technical_data['ma20'].iloc[-1]:.2f}")
                if 'rsi' in technical_data and len(technical_data['rsi']) > 0:
                    report.append(f"RSI: {technical_data['rsi'].iloc[-1]:.2f}")
            ...
            if sentiment_data:
                report.append("【情绪分析】")
                report.append(f"综合情绪评分: {sentiment_data.get('overall_score', 50)}/100")
                
                fund_flow = sentiment_data.get('fund_flow', {})
                if fund_flow:
                    report.append(f"资金流向: {fund_flow.get('status', '--')}")
            ...
            if risk_data:
                ...
                report.append(f"夏普比率: {risk_data.get('sharpe_ratio', 0):.2f}")
                report.append("")
            ...
            if prediction_data:
                ...
                report.append(f"短期预测: {short_term.get('direction', '--')} (置信度: {short_term.get('confidence', 0):.1%})")
                report.append(f"中期预测: {medium_term.get('direction', '--')} (置信度: {medium_term.get('confidence', 0):.1%})")
                report.append(f"长期预测: {long_term.get('direction', '--')} (置信度: {long_term.get('confidence', 0):.1%})")
                report.append("")
            ...
            overall_advice = self.generate_overall_advice(tech_score, sentiment_score, risk_score)
            report.append(f"投资建议: {overall_advice}")
            report.append("")
            
            report.append("【免责声明】")
            report.append("本报告仅供参考，不构成投资建议。")
            report.append("投资有风险，入市需谨慎。")
            report.append("请根据自身风险承受能力谨慎决策。")
...
class AnalysisThread(QThread):
    ...
    def run(self):
        try:
            analysis_data = self.data_processor.get_comprehensive_data(self.stock_code)
            if analysis_data:
                self.analysis_finished.emit(analysis_data)
            else:
                self.analysis_error.emit("无法获取股票数据")
        except Exception as e:
